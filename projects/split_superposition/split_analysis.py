...
def process_file(file_path):
    ...
    alpha1, alpha2 = extract_alphas_from_filename(filename)
    ...
    try:
        ...
        if alpha1 is not None and alpha2 is not None:
            y_theo = model_function(x_data, alpha1, alpha2)
            ...
            if residual_fit > residual_theo:
                ...
                print(f"フィッティングパラメータ: α={alpha:.3f}, β={beta:.3f}")
                print(f"理論パラメータ: α={alpha1:.3f}, β={alpha2:.3f}")
                print("!"*50 + "\n")
        ...
        def create_plot(include_theoretical=True):
            ...
            plt.scatter(x_data, y_data, marker='o', color='blue', 
                       label=f'Data $(\\alpha={alpha1:.1f}, \\beta={alpha2:.1f})$', alpha=0.6)
            ...
            plt.plot(x_data, y_fitted, linestyle='--', color='blue', 
                    label=f'Fit: $F_\\mathrm{{super}}^2 = {coef1:.4f}n^{{3-{alpha:.4f}}} + {coef2:.4f}n^{{3-{beta:.4f}}}$')
            
            # 理論値のプロット（オプション）
            if include_theoretical and alpha1 is not None and alpha2 is not None:
                y_theo = model_function(x_data, alpha1, alpha2)
                plt.plot(x_data, y_theo, linestyle='-', color='red', linewidth=1.5,
                        label=f'Theoretical: $F_\\mathrm{{super}}^2(\\alpha={alpha1:.1f}, \\beta={alpha2:.1f})$')
            ...
            # タイトルを下部に配置
            title = f'Log-log plot of $n$-$F_\\mathrm{{super}}^2$ ($\\alpha_1={alpha1:.1f}$, $\\alpha_2={alpha2:.1f}$)'
